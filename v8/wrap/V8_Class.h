...
public:
	static class_singleton& create(v8::Isolate* isolate)
	{
		...
		class_instances* singletons =
			static_cast<class_instances*>(isolate->GetData(V8_ISOLATE_DATA_SLOT));
		if (!singletons)
		{
			// No singletons map yet, create and store it
			singletons = new class_instances;
			isolate->SetData(V8_ISOLATE_DATA_SLOT, singletons);
		}
		...
		if (singletons)
		{
			...
			if (my_type < singletons->size())
			{
				...
				if (instance->type() == my_type)
				{
					...
					{
						delete singletons;
						isolate->SetData(V8_ISOLATE_DATA_SLOT, nullptr);
					}
				}
			}
		}
...
public:
	...
	set(char const *name, Attribute attribute, bool readonly = false)
	{
		...
		class_singleton_.class_function_template()->PrototypeTemplate()->SetAccessor(
			v8_wrap::to_v8(isolate(), name), getter, setter, data, v8::DEFAULT, prop_attrs);
		return *this;
	}
	...
	class_& set_const(char const* name, Value value)
	{
		...
		class_singleton_.class_function_template()->PrototypeTemplate()->Set(v8_wrap::to_v8(isolate(), name),
